...
const TRANSLATIONS = {
  ...
  hu: { ...BASE_TRANSLATION, lang_label: "Nyelv", upload_title: "Kép feltöltése", upload_sub: "Húzd ide vagy Ctrl+V", colors_title: "Színek", col_text: "Szöveg", col_bg: "Háttér", layout_title: "Elrendezés", font_label: "Betűtípus", btn_width: "Szélességhez", btn_height: "Magassághoz", lbl_cols: "Oszlopok", lbl_rows: "Sorok", lbl_fontsize: "Betűméret", lbl_lineheight: "Sormagasság", lbl_spacing: "Betűköz", adj_title: "Beállítások", adj_contrast: "Kontrast", adj_brightness: "Fényerő", adj_sharpness: "Élesség", adj_denoise: "Zajcsökkentés", adj_exposure: "Expozíció", lbl_invert: "Invertálás", char_title: "Karakterkészlet", char_placeholder: "Saját karakterek...", btn_char_standard: "Standard", btn_char_simple: "Egyszerű", crop_title: "Képvágás", btn_cancel: "Mégse", btn_done: "Kész", status_ready: "Kész", status_wait: "Tölts fel képet", empty_title: "Képre várva...", empty_sub: "Tölts fel egy képet balra.", btn_crop: "Vágás", btn_copy: "Másolás", export_title: "ASCII Art Exportálás" },
...
export default function App() {
  ...
  const t = TRANSLATIONS[lang] || TRANSLATIONS.en || BASE_TRANSLATION; 
  ...
  const copyToClipboard = () => navigator.clipboard.writeText(asciiArt);
  const downloadTxt = () => {
  ...
  const downloadJpg = () => {
    ...
    try {
      ...
      if (canvasWidth * canvasHeight > 100000000) {
          alert("Bild zu groß für Export. Bitte Auflösung reduzieren.");
          return;
      }
      ...
      canvas.toBlob((blob) => {
        if (!blob) return alert("Fehler beim Export.");
  ...
  const downloadHtml = () => {
    ...
    // SAFE SCALING LOGIC:
    // Browser erzwingen oft eine Mindestschriftgröße (z.B. 10px).
    // Wenn man 2px anfordert, wird mit 10px gerendert, aber die Zeilenhöhe bleibt bei 2px.
    // Das verursacht die schwarzen Balken (Überlappung).
    // Lösung: Wir rendern mit min. 12px und skalieren per CSS herunter.
    
    const SAFE_MIN_FONT_SIZE = 12;
    let exportFontSize = config.fontSize;
    let scaleFactor = 1;

    // Wenn die gewünschte Größe zu klein ist, skalieren wir.
    if (config.fontSize < SAFE_MIN_FONT_SIZE) {
        scaleFactor = config.fontSize / SAFE_MIN_FONT_SIZE;
        exportFontSize = SAFE_MIN_FONT_SIZE;
    }

    const exportLineHeight = exportFontSize * config.lineHeight;
    // Letter Spacing muss proportional angepasst werden
    // Wenn wir die Schrift 6x vergrößern (2px -> 12px), muss der Abstand auch 6x größer sein,
    // damit er nach dem Herunterskalieren wieder stimmt.
    const exportLetterSpacing = config.letterSpacing * (1 / scaleFactor);
...
<head>
    ...
    <style>
        ...
        .ascii-wrapper {
            ...
            /* Hier passiert die Magie: Wir skalieren das große Rendering visuell herunter */
        ...
        pre {
            ...
            letter-spacing: ${exportLetterSpacing}px;
...
<body>
    ...
    element.href = URL.createObjectURL(file);
    element.download = `ascii-art-${Date.now()}.html`;
    document.body.appendChild(element);
    element.click();
  ...
  return (
    <>
      <div className="ascii-studio">
        ...
        <div className={`ascii-overlay ${showSidebar ? 'ascii-overlay--visible' : ''}`} onClick={() => setShowSidebar(false)} />
        {isCropping && imageSrc && <CropEditor imageSrc={imageSrc} crop={tempCrop} onChange={setTempCrop} onComplete={() => { setCrop(tempCrop); setIsCropping(false); }} onCancel={() => setIsCropping(false)} t={t} />}
        ...
        <aside className={`ascii-sidebar ${showSidebar ? 'ascii-sidebar--open' : ''}`}>
          <div className="ascii-sidebar__header">
            ...
            <button className="ascii-sidebar__close-mobile" onClick={() => setShowSidebar(false)}><X className="w-6 h-6" /></button>
          </div>
          ...
          <div className="ascii-sidebar__content custom-scrollbar">
            <div className="ascii-controls__section">
               ...
               <select value={lang} onChange={(e) => setLang(e.target.value)} className="ascii-select">
                 {Object.keys(LANGUAGES).map(code => <option key={code} value={code}>{LANGUAGES[code]}</option>)}
               </select>
            </div>
            <div className="ascii-controls__section">
            ...
            <div className="ascii-controls__section">
              ...
              <div className="ascii-controls__group">
                ...
                <select value={config.fontKey} onChange={(e) => setConfig({...config, fontKey: e.target.value})} className="ascii-select">
                  {Object.entries(FONTS).map(([key, font]) => <option key={key} value={key}>{font.name}</option>)}
                </select>
              </div>
            ...
            <div className="ascii-controls__section pb-8">
              ...
              <textarea value={config.charSet} onChange={(e) => setConfig({...config, charSet: e.target.value})} className="w-full h-20 bg-slate-900 border border-slate-700 rounded p-2 text-xs font-mono text-slate-300 focus:border-emerald-500 outline-none resize-none mt-2" placeholder={t.char_placeholder} />
            </div>
          </div>
        </aside>
        ...
        <main className="ascii-main" style={{ backgroundColor: config.backgroundColor, transition: 'margin-left 0.3s' }}>
          <div className="ascii-toolbar">
            ...
            <div className="ascii-toolbar__right">
              {imageSrc && (
                <button onClick={() => setIsCropping(true)} className="ascii-button hide-on-mobile" title={t.btn_crop}><CropIcon className="w-4 h-4" /> <span>{t.btn_crop}</span></button>
              )}
              {imageSrc && (
                  <button onClick={() => setIsCropping(true)} className="ascii-button ascii-button--icon-only show-on-mobile" title={t.btn_crop}><CropIcon className="w-4 h-4" /></button>
              )}
              <button onClick={copyToClipboard} disabled={!asciiArt} className="ascii-button" title={t.btn_copy}><Copy className="w-4 h-4" /> <span className="hide-on-mobile">{t.btn_copy}</span></button>
              <button onClick={downloadJpg} disabled={!asciiArt} className="ascii-button" title="Download JPG"><FileImage className="w-4 h-4" /> <span className="hide-on-mobile">JPG</span></button>
              <button onClick={downloadHtml} disabled={!asciiArt} className="ascii-button" title="Download HTML"><FileCode className="w-4 h-4" /> <span className="hide-on-mobile">HTML</span></button>
              <button onClick={downloadTxt} disabled={!asciiArt} className="ascii-button" title="Download TXT"><Download className="w-4 h-4" /> <span className="hide-on-mobile">TXT</span></button>
            </div>
          </div>
          <div className="ascii-preview custom-scrollbar">
            <div className="ascii-preview__content">
              {asciiArt ? (
                <pre className="ascii-preview__pre" style={{ fontSize: `${config.fontSize}px`, lineHeight: `${config.fontSize * config.lineHeight}px`, letterSpacing: `${config.letterSpacing}px`, fontFamily: FONTS[config.fontKey].family, color: config.textColor }}>{asciiArt}</pre>
              ) : (
                <div className="ascii-preview__empty"><TypeIcon className="w-16 h-16 opacity-20 mb-4" /><p className="text-lg font-medium">{t.empty_title}</p><p className="text-sm opacity-60 mt-1">{t.empty_sub}</p></div>
              )}
            </div>
            {imageSrc && !isCropping && <div className="ascii-preview__overlay hide-on-mobile"><img src={imageSrc} alt="Original" /></div>}
          </div>
          {isProcessing && <div className="absolute inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50"><RefreshCw className="w-12 h-12 text-emerald-500 animate-spin" /></div>}
        </main>
      </div>
    </>
  );
