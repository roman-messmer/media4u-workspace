...
const els = {
    ...
    settingUiDist: document.getElementById('setting-ui-dist'),
    uiDistVal: document.getElementById('ui-dist-val'),
    vrLeft: null, 
    vrRight: null
};
    ...
    if (renderer.xr.isPresenting) {
        ...
        if (els.vrLeft && els.vrRight) {
            ...
            if (state.vrUiActive) {
                ...
                els.vrLeft.clone.style.transform = `translate(calc(-50% + ${shiftX + state.uiSeparation}px), -50%) scale(1.1)`;
                els.vrRight.clone.style.transform = `translate(calc(-50% + ${shiftX - state.uiSeparation}px), -50%) scale(1.1)`;
...
function createPlaceholderTexture() {
    ...
    ctx.fillText('360° VIEWER PRO', 512, 240); // Leicht nach oben geschoben

    // URL darunter
    ctx.fillStyle = '#ffffff'; // Farbe Weiss für einen schönen Kontrast
    ctx.font = '24px sans-serif'; // Etwas kleinere Schriftgrösse
    ctx.fillText('www.media4u.ch', 512, 280); // Darunter platziert
...
function initVrUi() {
    ...
    const createVrSide = (side) => {
        ...
        // Initialen Abstand direkt auf das Fadenkreuz anwenden
        reticle.style.transform = side === 'left' 
            ? `translate(calc(-50% + ${state.uiSeparation}px), -50%)` 
            : `translate(calc(-50% - ${state.uiSeparation}px), -50%)`;
...
els.settingUiDist.addEventListener('input', (e) => { 
    state.uiSeparation = parseInt(e.target.value, 10); 
    els.uiDistVal.textContent = e.target.value; 
    
    if (els.vrLeft && els.vrRight) {
        els.vrLeft.reticleFill.parentElement.style.transform = `translate(calc(-50% + ${state.uiSeparation}px), -50%)`;
        els.vrRight.reticleFill.parentElement.style.transform = `translate(calc(-50% - ${state.uiSeparation}px), -50%)`;
    }
});
