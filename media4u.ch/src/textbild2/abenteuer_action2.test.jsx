...
import AbenteuerAction2 from './abenteuer_action2';
...
vi.mock('react-i18next', () => ({
  useTranslation: () => ({ t: (key) => key === 'abenteuer_action2.title' ? 'Action 2 Dummy Titel' : key, i18n: { language: 'de', split: () => ['de'] } })
}));
...
describe('Abenteuer & Action 2 Kategorie', () => {
  beforeEach(() => { vi.clearAllMocks(); sessionStorage.clear(); });

  it('zeigt initial den Ladezustand an', () => {
    fetch.mockImplementation(() => new Promise(() => {}));
    render(<AbenteuerAction2 />);
  ...
  it('rendert die Textbilder nach erfolgreichem API-Aufruf', async () => {
    fetch.mockImplementation((url) => {
      ...
      if (url.includes('/api/textbild2s')) return Promise.resolve({ 
        ok: true, json: () => Promise.resolve({ data: [{ id: 1, attributes: { Link2: 'test-item-1', Name_Preview2: [{ Name_Preview_Deutsch: 'Mein Action Bild 2' }], Textfeldgruppe2: [{ Preview: true, Sprachen_Frame_Deutsch: '<div></div>' }], textbild2_kategorie: { data: [{ attributes: { Slug: 'abenteuer_action2' } }] } } }] }) 
      });
    ...
    render(<AbenteuerAction2 />);
    await waitFor(() => {
      expect(screen.getByText('Action 2 Dummy Titel')).toBeInTheDocument();
      expect(screen.getAllByText('Mein Action Bild 2')[0]).toBeInTheDocument();
    });
  });
});