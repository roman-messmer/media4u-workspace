...
import ErotikSex2 from './erotik_sex2';
...
vi.mock('react-i18next', () => ({
  useTranslation: () => ({ t: (key) => key === 'erotik_sex2.title' ? 'Erotik 2 Dummy Titel' : key, i18n: { language: 'de', split: () => ['de'] } })
}));
...
describe('Erotik & Sex 2 Kategorie', () => {
  ...
  it('rendert die Textbilder nach erfolgreichem API-Aufruf', async () => {
    fetch.mockImplementation((url) => {
      ...
      if (url.includes('/api/textbild2s')) return Promise.resolve({ 
        ok: true, json: () => Promise.resolve({ data: [{ id: 1, attributes: { Link2: 'test-item-1', Name_Preview2: [{ Name_Preview_Deutsch: 'Mein Erotik Bild 2' }], Textfeldgruppe2: [{ Preview: true, Sprachen_Frame_Deutsch: '<div></div>' }], textbild2_kategorie: { data: [{ attributes: { Slug: 'erotik_sex2' } }] } } }] }) 
      });
    ...
    render(<ErotikSex2 />);
    await waitFor(() => {
      expect(screen.getByText('Erotik 2 Dummy Titel')).toBeInTheDocument();
      expect(screen.getAllByText('Mein Erotik Bild 2')[0]).toBeInTheDocument();
    });
  });
});