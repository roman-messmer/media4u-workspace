// src/components/content/textbild2/erotik_sex2.jsx
...
const SmsAnimation = ({ item, baseLangKey }) => {
    ...
    if (!item) return <div>{i18n.t('erotik_sex2.default_message') || "Kein Bild ausgewählt."}</div>;
    const attrs = item.attributes || item;
    const gruppe = attrs.Textfeldgruppe2 || attrs.textfeldgruppe2 || attrs.Textfeldgruppe || attrs.textfeldgruppe || [];
        ...
        const fetchSmsTextbilder = async () => {
            try {
                ...
                const filtered = allData.filter(item => {
                    ...
                    return catArray.some(c => (c.attributes?.Slug || c.Slug || c.slug) === 'erotik_sex2');
                });
                setApiData(filtered);
    ...
    const allVorschauComponents = apiData.map((item) => {
        ...
        return (
            <figure key={item.id || item.documentId} className="vorschau_textbild2">
                  ...
                  <figcaption>
                    <Link to={`/textbild2/erotik_sex2/${itemSlug}`} className="vorschau_name">{finalName}</Link>
                  </figcaption>
            </figure>
        );
    }).filter(c => c !== null);
