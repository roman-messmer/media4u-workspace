// src/components/content/textbild2/liebe_freundschaft2.jsx
...
const SmsAnimation = ({ item, baseLangKey }) => {
    ...
    if (!item) return <div>{i18n.t('liebe_freundschaft2.default_message') || "Kein Bild ausgewählt."}</div>;
...
const LiebeFreundschaft2 = () => {
        ...
        const fetchSmsTextbilder = async () => {
            try {
                ...
                const filtered = allData.filter(item => {
                    const attrs = item.attributes || item;
                    let cats = attrs.textbild_2_kategorie?.data || attrs.textbild2_kategorie?.data || attrs.textbild2Kategorie || attrs.textbild2_kategories?.data; 
                    ...
                    return catArray.some(c => (c.attributes?.Slug || c.Slug || c.slug) === 'liebe_freundschaft2');
                });
                setApiData(filtered);
    ...
    const allVorschauComponents = apiData.map((item) => {
        ...
        return (
            <figure key={item.id || item.documentId} className="vorschau_textbild2">
                  ...
                  <figcaption>
                    <Link to={`/textbild2/liebe_freundschaft2/${itemSlug}`} className="vorschau_name">{finalName}</Link>
                  </figcaption>
            </figure>
        );
    }).filter(c => c !== null);
    ...
    return (
        <div className="liebe_freundschaft2">
            ...
            {allVorschauComponents.length > itemsPerPage && (
                <div className="pagination">
                    <button onClick={() => paginate(currentPage - 1)} disabled={currentPage === 1}>&laquo; Zurück</button>
                    {[...Array(Math.ceil(allVorschauComponents.length / itemsPerPage))].map((_, i) => (
                        <button key={i} onClick={() => paginate(i + 1)} className={currentPage === i + 1 ? 'active' : ''}>{i + 1}</button>
                    ))}
                    <button onClick={() => paginate(currentPage + 1)} disabled={currentPage === Math.ceil(allVorschauComponents.length / itemsPerPage)}>Weiter &raquo;</button>
                </div>
            )}
        </div>
    );
...
export default LiebeFreundschaft2;