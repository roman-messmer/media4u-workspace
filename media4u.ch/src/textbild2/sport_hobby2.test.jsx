...
import SportHobby2 from './sport_hobby2';
...
vi.mock('react-i18next', () => ({
  useTranslation: () => ({
    t: (key) => key === 'sport_hobby2.title' ? 'Sport 2 Dummy Titel' : key,
...
describe('Sport & Hobby 2 Kategorie', () => {
  ...
  it('rendert die Textbilder nach erfolgreichem API-Aufruf', async () => {
      ...
      if (url.includes('/api/textbild2s')) return Promise.resolve({ 
        ...
        json: () => Promise.resolve({ 
          data: [{ 
            ...
            attributes: { 
              Link2: 'test-item-1', 
              Name_Preview2: [{ Name_Preview_Deutsch: 'Mein Sport Bild 2' }], 
              Textfeldgruppe2: [{ Preview: true, Sprachen_Frame_Deutsch: '<div></div>' }],
              textbild2_kategorie: { data: [{ attributes: { Slug: 'sport_hobby2' } }] }
            } 
          }] 
        }) 
      });
    ...
    await waitFor(() => {
      expect(screen.getByText('Sport 2 Dummy Titel')).toBeInTheDocument();
      expect(screen.getAllByText('Mein Sport Bild 2')[0]).toBeInTheDocument();
