...
import ErotikSex from './erotik_sex';
...
describe('Erotik & Sex Kategorie', () => {
  ...
  it('rendert die Kategorie nach erfolgreichem API-Aufruf', async () => {
    fetch.mockImplementation((url) => {
      ...
      if (url.includes('/api/sms-textbild-kategories')) return Promise.resolve({ ok: true, json: () => Promise.resolve({ data: [{ attributes: { Name: 'Erotik Dummy Kategorie' } }] }) });
      if (url.includes('/api/sms-textbilds')) return Promise.resolve({ ok: true, json: () => Promise.resolve({ data: [{ id: 1, attributes: { Link: 'test', Name_Preview: [{ Name_Preview_Deutsch: 'Mein Erotik Bild' }], Textfeldgruppe: [{ Preview: true, Sprachen_Frame_Deutsch: '<div></div>' }] } }] }) });
    ...
    await waitFor(() => {
      expect(screen.getByText('Erotik Dummy Kategorie')).toBeInTheDocument();
      expect(screen.getByText('Mein Erotik Bild')).toBeInTheDocument();
    });
  });
});