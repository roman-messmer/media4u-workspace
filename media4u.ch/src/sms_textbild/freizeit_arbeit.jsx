// src/sms_textbild/freizeit_arbeit.jsx
...
const FreizeitArbeit = () => {
    ...
    // API Fetch (Parallel & Server-Filter für Freizeit & Arbeit)
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            try {
                const [catRes, itemRes] = await Promise.all([
                    fetch(`${import.meta.env.VITE_STRAPI_URL}/api/sms-textbild-kategories?filters[Slug][$eq]=freizeit_arbeit&locale=${locale}`, { signal: controller.signal }),
                    fetch(`${import.meta.env.VITE_STRAPI_URL}/api/sms-textbilds?populate=*&filters[Animation_Status][$eq]=online&filters[sms_textbild_kategorie][Slug][$eq]=freizeit_arbeit&sort[0]=createdAt:desc&pagination[pageSize]=100`, { signal: controller.signal })
                ]);
    ...
    // Kugelsicheres Matching für Direktaufrufe (URL-Link)
    ...
    // Galerie-Mapping (Vorschau-Elemente ohne DOMPurify-Konflikt)
    const validItems = apiData.map(item => {
        ...
        return (
            <figure key={item.id} className="vorschau_sms_textbild">
                ...
                <figcaption>
                    <Link to={`/sms_textbild/freizeit_arbeit/${attrs.Link || item.id}`} className="vorschau_name">
            ...
            <div className="display_area zoom-in">
                {activeItem ? (
                    ...
                    <div className="sms_default_message">
                        {categoryInfo?.Beschreibung || categoryInfo?.description || t('freizeit_arbeit.default_message')}
                    </div>
                )}
            </div>
            
            <h1 className="category_title">{categoryInfo?.Name || "Freizeit & Arbeit"}</h1>
