// src/sms_textbild/erotik_sex.jsx
...
// --- 1. HILFSFUNKTIONEN (Verschlankt & Robust) ---
...
const ErotikSex = () => {
    ...
    // API Fetch (Parallel & Server-Filter für erotik_sex)
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            try {
                const [catRes, itemRes] = await Promise.all([
                    fetch(`${import.meta.env.VITE_STRAPI_URL}/api/sms-textbild-kategories?filters[Slug][$eq]=erotik_sex&locale=${locale}`, { signal: controller.signal }),
                    fetch(`${import.meta.env.VITE_STRAPI_URL}/api/sms-textbilds?populate=*&filters[Animation_Status][$eq]=online&filters[sms_textbild_kategorie][Slug][$eq]=erotik_sex&sort[0]=createdAt:desc&pagination[pageSize]=100`, { signal: controller.signal })
                ]);
    ...
    // Kugelsicheres Matching für das aktive Bild
    ...
    const validItems = apiData.map(item => {
        ...
        return (
            <figure key={item.id} className="vorschau_sms_textbild">
                ...
                <figcaption>
                    <Link to={`/sms_textbild/erotik_sex/${attrs.Link || item.id}`} className="vorschau_name">
            ...
            <div className="display_area zoom-in">
                {activeItem ? (
                    ...
                    <div className="sms_default_message">
                        {categoryInfo?.Beschreibung || categoryInfo?.description || t('erotik_sex.default_message')}
                    </div>
                )}
            </div>
            
            <h1 className="category_title">{categoryInfo?.Name || "Erotik & Sex"}</h1>
