// src/sms_textbild/abenteuer_action.jsx
...
const AbenteuerAction = () => {
    ...
    // API Fetch (Parallel & Server-Filter für abenteuer_action)
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            try {
                const [catRes, itemRes] = await Promise.all([
                    fetch(`${import.meta.env.VITE_STRAPI_URL}/api/sms-textbild-kategories?filters[Slug][$eq]=abenteuer_action&locale=${locale}`, { signal: controller.signal }),
                    fetch(`${import.meta.env.VITE_STRAPI_URL}/api/sms-textbilds?populate=*&filters[Animation_Status][$eq]=online&filters[sms_textbild_kategorie][Slug][$eq]=abenteuer_action&sort[0]=createdAt:desc&pagination[pageSize]: '100'`, { signal: controller.signal })
                ]);
    ...
    // Kugelsicheres Matching für das aktive Bild (für Direktaufrufe)
    ...
    const validItems = apiData.map(item => {
        ...
        return (
            <figure key={item.id} className="vorschau_sms_textbild">
                ...
                <figcaption>
                    <Link to={`/sms_textbild/abenteuer_action/${attrs.Link || item.id}`} className="vorschau_name">
    ...
    return (
        <div className="abenteuer_action">
            ...
            <div className="display_area zoom-in">
                {activeItem ? (
                    ...
                    <div className="sms_default_message">
                        {categoryInfo?.Beschreibung || categoryInfo?.description || t('abenteuer_action.default_message')}
                    </div>
                )}
            </div>
            
            <h1 className="category_title">{categoryInfo?.Name || "Abenteuer & Action"}</h1>
...
export default AbenteuerAction;