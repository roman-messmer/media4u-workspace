// ./src/components/content/smstextbild/neu.jsx
...
// Die NEUE, smarte Werbungskomponente (Pfad ggf. anpassen)
import Werbung from '../werbung/werbung.jsx';

// --- HILFSFUNKTIONEN (Außerhalb der Komponente) ---

// 1. Feldname für den Inhalt ermitteln
...
// 2. Feldname für den Anzeigenamen ermitteln
...
// 3. Name aus dem Item extrahieren
const getItemName = (item, nameFieldKey) => {
    ...
    return localizedName || attrs.Name || attrs.name || 'Textbild';
};

// 4. Wrapper <div class="frame"> entfernen
...
const Neu = () => {
    ...
    // 1. Eigener, DSGVO-konformer IP Check (MaxMind via Strapi)
    useEffect(() => {
        const fetchCountry = async () => {
            ...
            try {
                // Hier rufst du dein eigenes Backend auf (Endpunkt ggf. anpassen)
    ...
    // 2. Fetch Strapi Data (SMS Textbilder)
    useEffect(() => {
        ...
        const fetchSmsTextbilder = async () => {
            try {
                const queryParams = new URLSearchParams({
                    ...
                    'filters[Animation_Status][$eq]': 'online'
                });

                const response = await fetch(`${import.meta.env.VITE_STRAPI_URL}/api/sms-textbilds?${queryParams.toString()}`, { signal });
    ...
    const allVorschauComponents = apiData.map((item) => {
        ...
        const categories = attrs.sms_textbild_kategorie?.data || attrs.sms_textbild_kategorie || attrs.smsTextbildKategorie;
        const firstCategory = Array.isArray(categories) && categories.length > 0 
            ? categories[0] 
            : (categories?.data && categories.data.length > 0 ? categories.data[0] : null);
            
        const categorySlug = firstCategory ? (firstCategory.attributes?.Slug || firstCategory.Slug || firstCategory.slug) : 'allgemein';
        const itemSlug = attrs.Link || attrs.link || attrs.Slug || attrs.slug || item.id || item.documentId;

        return (
            <figure key={item.id || item.documentId} className="vorschau_sms_textbild">
                 <div className="display deselect">
                    <div className="frame" dangerouslySetInnerHTML={{ __html: sanitizedContent }} />
                 </div>
                 <figcaption>
                    <Link to={`/sms_textbild/${categorySlug}/${itemSlug}`} className="vorschau_name">
                        {finalName}
                    </Link>
                 </figcaption>
            </figure>
    ...
    return (
        <>
            {/* HIER sitzt jetzt die neue, smarte Werbungskomponente und nimmt das Land entgegen */}
