// src/sms_textbild/festtage_party.jsx
...
const FesttageParty = () => {
    ...
    // API Fetch (Parallelisiert & Server-Filter für festtage_party)
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            try {
                const [catRes, itemRes] = await Promise.all([
                    fetch(`${import.meta.env.VITE_STRAPI_URL}/api/sms-textbild-kategories?filters[Slug][$eq]=festtage_party&locale=${locale}`, { signal: controller.signal }),
                    fetch(`${import.meta.env.VITE_STRAPI_URL}/api/sms-textbilds?populate=*&filters[Animation_Status][$eq]=online&filters[sms_textbild_kategorie][Slug][$eq]=festtage_party&sort[0]=createdAt:desc&pagination[pageSize]=100`, { signal: controller.signal })
                ]);
    ...
    // Kugelsicheres Matching für das aktive Bild (für Deep-Links)
    ...
    const validItems = apiData.map(item => {
        ...
        return (
            <figure key={item.id} className="vorschau_sms_textbild">
                ...
                <figcaption>
                    <Link to={`/sms_textbild/festtage_party/${attrs.Link || item.id}`} className="vorschau_name">
            ...
            <div className="display_area zoom-in">
                {activeItem ? (
                    ...
                    <div className="sms_default_message">
                        {categoryInfo?.Beschreibung || categoryInfo?.description || t('festtage_party.default_message')}
                    </div>
                )}
            </div>
            
            <h1 className="category_title">{categoryInfo?.Name || "Festtage & Party"}</h1>
