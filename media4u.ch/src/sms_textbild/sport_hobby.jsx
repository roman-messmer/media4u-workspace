// src/sms_textbild/sport_hobby.jsx
...
// --- 1. HILFSFUNKTIONEN (Zentralisiert & Sprach-resilient) ---
...
const SportHobby = () => {
    ...
    // API Fetch (Parallelisiert & Server-seitig gefiltert)
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            try {
                const [catRes, itemRes] = await Promise.all([
                    fetch(`${import.meta.env.VITE_STRAPI_URL}/api/sms-textbild-kategories?filters[Slug][$eq]=sport_hobby&locale=${locale}`, { signal: controller.signal }),
                    fetch(`${import.meta.env.VITE_STRAPI_URL}/api/sms-textbilds?populate=*&filters[Animation_Status][$eq]=online&filters[sms_textbild_kategorie][Slug][$eq]=sport_hobby&sort[0]=createdAt:desc&pagination[pageSize]=100`, { signal: controller.signal })
                ]);
    ...
    // Kugelsicheres Matching für das aktive Bild (Löst dein Problem beim Direktaufruf)
    const activeItem = useMemo(() => {
        if (loading || !apiData.length) return null;

        // Fallback auf das erste Element, wenn displayId fehlt
        const searchId = String(displayId || apiData[0]?.attributes?.Link || apiData[0]?.id || "").toLowerCase();

        return apiData.find(item => {
            const attr = item.attributes || item;
            const matches = [
                attr.Link, attr.link, attr.Slug, attr.slug, 
    ...
    // Galerie-Mapping (Vorschau-Elemente)
    const validItems = apiData.map(item => {
        ...
        return (
            <figure key={item.id} className="vorschau_sms_textbild">
                ...
                <figcaption>
                    <Link to={`/sms_textbild/sport_hobby/${attrs.Link || item.id}`} className="vorschau_name">
            ...
            <div className="display_area zoom-in">
                {activeItem ? (
                    ...
                    <div className="sms_default_message">
                        {/* Kategorie-Beschreibung aus Strapi */}
                        {categoryInfo?.Beschreibung || categoryInfo?.description || t('sport_hobby.default_message')}
                    </div>
                )}
            </div>
            
            <h1 className="category_title">{categoryInfo?.Name || "Sport & Hobby"}</h1>
