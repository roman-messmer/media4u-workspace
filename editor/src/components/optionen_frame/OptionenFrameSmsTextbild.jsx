// src/components/optionen_frame/OptionenFrameSmsTextbild.jsx
...
export default function OptionenFrameSmsTextbild({ onAction }) {
  const listRef = useRef(null)
  const [canLeft, setCanLeft] = useState(false)
  const [canRight, setCanRight] = useState(false)
  const [confirmOpen, setConfirmOpen] = useState(false)
  const { showStrapiSMSTextbild, toggleStrapiSMSTextbild } = useUiToggles()

  const config = [
    { id: 'new',    label: 'Neuer Frame',       icon: icNewspaper },
  ...
  const scrollNav = (direction) => {
    ...
    const next = direction === 'left'
      ...
      : Math.min(maxScrollLeft, current + step)
  ...
  return (
    <nav className="nav_optionen_frame" aria-label="Optionen Frame" role="toolbar">
      ...
      <ul ref={listRef} aria-label="Frame Aktionen" tabIndex={0}>
        {config.map(item => {
          const active = item.id === 'strapi' && showStrapiSMSTextbild
          return (
            <li key={item.id}>
              <button
                type="button"
